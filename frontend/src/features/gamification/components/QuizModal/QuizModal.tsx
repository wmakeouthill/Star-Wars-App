...
function buildQuizFromCache(params: {
  ...
  const { queryClient, questionCount, optionsCount, seed } = params;
  ...
  const generators: Array<() => QuizQuestion | null> = [
    // Filmes: Diretor
    () => {
    ...
    // Personagens: Planeta natal (se veio no payload)
    () => {
      const eligible = characters.filter(
        (c) => normalizeText(c.homeworld?.name) && normalizeText(c.name)
      );
      const picked = eligible[Math.floor(rng() * eligible.length)];
      if (!picked) return null;
      return createQuestion({
    ...
    body = (
      <div className={styles.finish}>
        ...
        <div className={styles.finishActions}>
          <button type="button" className={styles.primaryButton} onClick={regenerate}>
            Jogar de novo
          </button>
        </div>
      </div>
        ...
        <div className={styles.topBar}>
          <div className={styles.status}>
            {total > 0 ? (
              ...
              <span className={styles.badge}>Sem perguntas (cache vazio)</span>
            )}
          </div>

          <div className={styles.actions}>
            <button type="button" className={styles.secondaryButton} onClick={regenerate}>
              Recarregar
            </button>
          </div>
        </div>
